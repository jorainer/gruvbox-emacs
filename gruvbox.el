...
;;; Commentary:
;;
;; A port of gruvbox to Emacs.
;;
;;; Installation:
;;
;;   Drop the `gruvbox-theme.el` somewhere in your `load-path` and
...
;;; Credits
;;
;; Bozhidar Batsov for the solarized-emacs theme on which this code is based.
;;
;;; Code:
...
(defcustom gruvbox-high-contrast-code-block-header nil
  "Use a high contrast background color for source code blocks."
  :type 'boolean
  :group 'gruvbox
  )

(defcustom gruvbox-distinct-fringe-background nil
  "Make the fringe background different from the normal background color.
Also affects `linum-mode' background."
  :type 'boolean
  :group 'gruvbox)

(defcustom gruvbox-distinct-doc-face nil
...
(defcustom gruvbox-use-less-bold nil
  "Use bold weight less often."
  :type 'boolean
  :group 'gruvbox)

(defcustom gruvbox-use-more-italic nil
  "Use italic slant more often."
  :type 'boolean
  :group 'gruvbox)

(defcustom gruvbox-emphasize-indicators t
  "Use more colors for indicators such as git:gutter, flycheck and similar."
  :type 'boolean
  :group 'gruvbox)

(defcustom gruvbox-high-contrast-mode-line nil
...
(defcustom gruvbox-height-plus-2 1.15
  "Font size +2."
  :type 'number
  :group 'gruvbox)

(defcustom gruvbox-height-plus-3 1.2
  "Font size +3."
  :type 'number
  :group 'gruvbox)

(defcustom gruvbox-height-plus-4 1.3
  "Font size +4."
  :type 'number
  :group 'gruvbox)

(defcustom gruvbox-scale-headlines t
  "Whether scaling of outline-headlines should apply to headlines."
  :type 'boolean
  :group 'gruvbox)

;;; Utilities

(defun gruvbox-color-name-to-rgb (color &optional frame)
...
(defun gruvbox-color-rgb-to-hex  (red green blue)
  "Return hexadecimal notation for the color RED GREEN BLUE.
RED, GREEN, and BLUE should be numbers between 0.0 and 1.0, inclusive."
  (format "#%02x%02x%02x"
          (* red 255) (* green 255) (* blue 255)))

(defun gruvbox-color-blend (color1 color2 alpha)
...
Alpha should be a float between 0 and 1."
  (apply 'gruvbox-color-rgb-to-hex
         (-zip-with '(lambda (it other)
                       (+ (* alpha it) (* other (- 1 alpha))))
                    (gruvbox-color-name-to-rgb color1)
                    (gruvbox-color-name-to-rgb color2))))

;;; The colors from gruvbox (come always in lighter, darker shade):
;; To compare colors etc see http://designmodo.github.io/Flat-UI/ and
;; http://ethanschoonover.com/solarized

;;; Setup Start
(defmacro gruvbox-with-color-variables (variant &rest body)
  (declare (indent 0))
  `(let* ((class '((class color) (min-colors 89)))
          ...
          ;; (s-base02    wetasphalt)
          ;; ;; emphasized content; light-dark grey/blue
          ;; (s-base01    asbestos)
          ;; ;; primary content; grey/blue and light grey (text color?)
          ;; (s-base00    asbestos)
          ;; (s-base0     concrete)
          ;; ;; comments; even brighter.
          ;; (s-base1     concrete)
          ;; ;; background highlight light; light yellow
          ;; (s-base2     silver)
          ;; ;; background light; slightly lighter than above.
          ;; (s-base3     clouds)

          ;; Gruvbox accented colors
          ;; yellow -> sun flower
          ;; orange -> carrot
          ;; red -> alizarin
          ;; magenta -> amethyst
          ...
          ;; cyan -> turquoise
          ;; green -> emerald
          (yellow (if (eq variant 'light) l_yellow_3 d_yellow_3))
          (orange (if (eq variant 'light) l_orange_1 d_orange_2))
          (red (if (eq variant 'light) l_orange_1 d_orange_1))
          (magenta (if (eq variant 'light) magenta magenta))
          (violet (if (eq variant 'light) l_purple_5 d_purple_5))
          (blue (if (eq variant 'light) l_blue_4 d_blue_4))
          (cyan (if (eq variant 'light) l_aqua_6 d_aqua_6))
          (green (if (eq variant 'light) l_green_2 d_green_2))

          ;; Darker and lighter accented colors
          ;; Only use these in exceptional circumstances!
          (yellow-d   d_yellow_11)
          (yellow-l   l_yellow_11)
          (orange-d   d_orange_2)
          (orange-l   l_orange_2)
          (red-d      d_red_9)
          (red-l      l_red_9)
          (magenta-d  magenta)
          (magenta-l  magenta)
          (violet-d   d_purple_13)
          (violet-l   l_purple_13)
          (blue-d     d_blue_12)
          (blue-l     l_blue_12)
          (cyan-d     d_aqua_14)
          (cyan-l     l_aqua_14)
          ...
          ;; Gruvbox palette names, use these instead of -fg -bg...
          ;; Note: base03 and base02 are background tones for dark.
          ;;       base2 and base3 are background tones for bright.
          ...
          (base0 (if (eq variant 'light) l_fg_15 d_fg_15))
          (base00 (if (eq variant 'light) d_fg_15 l_fg_15))
          (base1 (if (eq variant 'light) l_fg3 d_fg3))
          (base01 (if (eq variant 'light) d_fg3 l_fg3))
          (base2 (if (eq variant 'light) l_fg2 d_fg2))
          (base02 (if (eq variant 'light) d_fg2 l_fg2))
          (base3 (if (eq variant 'light) l_fg_15 d_fg_15))
          ;; main background
          (base03 (if (eq variant 'light) l_bg_0 d_bg_0))
          ;; high contrast alternative:
          ;; (base03 (if (eq variant 'light) l_bg0_0 d_bg0_0))
          ...
          (s-line (if (eq variant 'light) d_fg4 l_fg4))
          ...
          (s-maybe-bold (if gruvbox-use-less-bold
                            'unspecified 'bold))
          (s-maybe-italic (if gruvbox-use-more-italic
                              'italic 'normal))
          (s-variable-pitch (if gruvbox-use-variable-pitch
                                'variable-pitch 'default))
          (s-fringe-bg (if gruvbox-distinct-fringe-background
                           base02 base03))
          (s-fringe-fg base01)

          (s-source-block-header-fg (if gruvbox-high-contrast-code-block-header
                                        base03 base01))

          (s-source-block-header-bg (if gruvbox-high-contrast-code-block-header
                                        base01 base03))

          (s-source-block-header-line (if gruvbox-high-contrast-code-block-header
                                          nil base01))

          (s-header-line-fg (if gruvbox-high-contrast-mode-line
                                base1 base0))
          (s-header-line-bg (if gruvbox-high-contrast-mode-line
                                base02 base03))
          (s-header-line-underline (if gruvbox-high-contrast-mode-line
                                       nil base02))

          (s-mode-line-fg (if gruvbox-high-contrast-mode-line
                              base03 base0))
          (s-mode-line-bg (if gruvbox-high-contrast-mode-line
                              base0 base02))
          (s-mode-line-underline (if gruvbox-high-contrast-mode-line
                                     nil s-line))

          (s-mode-line-buffer-id-fg (if gruvbox-high-contrast-mode-line
                                        'unspecified base1))
          (s-mode-line-inactive-fg (if gruvbox-high-contrast-mode-line
                                       base0 base01))
          (s-mode-line-inactive-bg (if gruvbox-high-contrast-mode-line
                                       base02 base03))
          (s-mode-line-inactive-bc (if gruvbox-high-contrast-mode-line
                                       base02 base02))
          )
     ,@body))

(defun create-gruvbox-theme (variant theme-name &optional childtheme)
...
;;;;; font lock
    ...
    `(font-lock-doc-face ((,class (:foreground ,(if gruvbox-distinct-doc-face violet cyan)
                                               :slant ,s-maybe-italic))))
...
;;;;; auctex
    ...
    `(font-latex-slide-title-face ((,class (:inherit (,s-variable-pitch font-lock-type-face)
                                                     :weight bold :height ,gruvbox-height-plus-3))))
    `(font-latex-string-face ((,class (:foreground ,cyan))))
    `(font-latex-subscript-face ((,class (:height ,gruvbox-height-minus-1))))
    `(font-latex-superscript-face ((,class (:height ,gruvbox-height-minus-1))))
...
;;;;; calfw
    ...
    `(cfw:face-title ((,class (:inherit ,s-variable-pitch :foreground ,yellow
                                        :weight bold :height ,gruvbox-height-plus-4))))
...
;;;;; custom
    `(custom-face-tag ((,class (:inherit ,s-variable-pitch :height ,gruvbox-height-plus-3
                                         :foreground ,violet :weight normal))))
    `(custom-variable-tag ((,class (:inherit ,s-variable-pitch
                                             :foreground ,cyan :height ,gruvbox-height-plus-3))))
    `(custom-comment-tag ((,class (:foreground ,base01))))
    `(custom-group-tag ((,class (:inherit ,s-variable-pitch :foreground ,blue :height ,gruvbox-height-plus-3))))
    `(custom-group-tag-1 ((,class (:inherit ,s-variable-pitch :foreground ,red :height ,gruvbox-height-plus-3))))
...
;;;;; diff
    ...
    `(diff-refine-added
      ((((class color) (background light))
        (:background ,(gruvbox-color-blend "#ddffdd" green 0.7)))
       (((class color) (background dark))
        (:background ,(gruvbox-color-blend "#446644" green 0.7)))))
    `(diff-refine-changed
      ((((class color) (background light))
        (:background ,(gruvbox-color-blend "#ddddff" blue 0.7)))
       (((class color) (background dark))
        (:background ,(gruvbox-color-blend "#444466" blue 0.7)))))
    `(diff-refine-removed
      ((((class color) (background light))
        (:background ,(gruvbox-color-blend "#ffdddd" red 0.7)))
       (((class color) (background dark))
        (:background ,(gruvbox-color-blend "#664444" red 0.7)))))
...
;;;;;; alternative ediff (not finished)
    ...
    ;;                               :background ,(gruvbox-color-blend blue base03 0.25))
    ;;                              )))
    ;; `(ediff-fine-diff-B ((,class (
    ;;                               :background ,(gruvbox-color-blend violet base03 0.25))
    ;;                              )))
    ;; `(ediff-fine-diff-C ((,class (
    ;;                               :background ,(gruvbox-color-blend yellow base03 0.25))
    ;;                              )))
    ;; `(ediff-current-diff-A ((,class (
    ;;                                  :background ,(gruvbox-color-blend blue base03 0.15)
    ;;                                              ))))
    ;; `(ediff-current-diff-B ((,class (
    ;;                                   :background ,(gruvbox-color-blend violet base03 0.15)
    ;;                                              ))))
    ;; `(ediff-current-diff-C ((,class (
    ;;                                  :background ,(gruvbox-color-blend yellow base03 0.15)
    ;;                                              ))))
    ;; `(ediff-even-diff-A ((,class (
    ;;                                ;; :background ,(gruvbox-color-blend base0 base03 0.15)
    ;;                               :background ,base02
    ;;                               ;; :foreground ,base2
    ;;                                ;; :background ,(gruvbox-color-blend green base02 0.15)
    ;;                                           ))))
...
;;;;; flycheck
    ...
    `(flycheck-info
      ((,(append '((supports :underline (:style wave))) class)
        (:underline (:style wave :color ,(if gruvbox-emphasize-indicators
                                             blue base03)) :inherit unspecified))
       (,class (:foreground ,blue-hc :background ,blue-lc :weight bold :underline t))))
    `(flycheck-fringe-error
      ((,class (:foreground ,(if gruvbox-emphasize-indicators
                                 red-hc red)
                            :background ,(if gruvbox-emphasize-indicators
...
;;;;; git-gutter
    `(git-gutter:added
      ((,class (:weight normal
                        :foreground ,(if gruvbox-emphasize-indicators
                                         green s-fringe-fg)
...
;;;;;; go-mode
    ...
    `(go-coverage-1 ((,class (:foreground ,(gruvbox-color-blend blue yellow (/ 2.0 6))))))
    `(go-coverage-2 ((,class (:foreground ,(gruvbox-color-blend blue yellow (/ 3.0 6))))))
    `(go-coverage-3 ((,class (:foreground ,(gruvbox-color-blend blue yellow (/ 4.0 6))))))
    `(go-coverage-4 ((,class (:foreground ,(gruvbox-color-blend blue yellow (/ 5.0 6))))))
    `(go-coverage-5 ((,class (:foreground ,blue))))
    `(go-coverage-6 ((,class (:foreground ,(gruvbox-color-blend cyan blue (/ 2.0 6))))))
    `(go-coverage-7 ((,class (:foreground ,(gruvbox-color-blend cyan blue (/ 3.0 6))))))
    `(go-coverage-8 ((,class (:foreground ,(gruvbox-color-blend cyan blue (/ 4.0 6))))))
    `(go-coverage-9 ((,class (:foreground ,(gruvbox-color-blend cyan blue (/ 5.0 6))))))
...
;;;;; hi-lock-mode
    `(hi-yellow ((,class (:foreground ,(gruvbox-color-blend yellow base1 0.5)
                                      :background,(gruvbox-color-blend yellow base03 0.15)))))
    `(hi-pink ((,class (:foreground ,(gruvbox-color-blend magenta base1 0.5)
                                    :background,(gruvbox-color-blend magenta base03 0.15)))))
    `(hi-green ((,class (:foreground ,(gruvbox-color-blend green base1 0.5)
                                     :background,(gruvbox-color-blend green base03 0.15)))))
    `(hi-blue ((,class (:foreground ,(gruvbox-color-blend blue base1 0.5)
                                    :background,(gruvbox-color-blend blue base03 0.15)))))
    `(hi-black-b ((,class (:foreground ,base1
                                       :background ,base03
                                       :weight bold))))
    `(hi-blue-b ((,class (:weight bold
                                  :foreground ,(gruvbox-color-blend cyan base1 0.7)
                                  :background ,(gruvbox-color-blend cyan base03 0.2)))))
    `(hi-green-b ((,class (:weight bold
                                   :foreground ,(gruvbox-color-blend green base1 0.7)
                                   :background ,(gruvbox-color-blend green base03 0.2)))))
    `(hi-red-b ((,class (:weight bold
                                 :foreground ,(gruvbox-color-blend red base1 0.7)
                                 :background ,(gruvbox-color-blend red base03 0.2)))))
...
;;;;;; headings and diffs
    ...
    `(magit-diff-hunk-heading
      ((t (:background ,(gruvbox-color-blend yellow base03 0.1)))))
    `(magit-diff-hunk-heading-highlight
      ((t (:background ,(gruvbox-color-blend yellow base02 0.1)))))
...
;;;;; markdown-mode
    ...
    `(markdown-header-face-1
      ((,class (:inherit ,s-variable-pitch :foreground ,blue
                         ,@(when gruvbox-scale-headlines
                             (list :height gruvbox-height-plus-3))))))
    `(markdown-header-face-2
      ((,class (:inherit ,s-variable-pitch :foreground ,violet
                         ,@(when gruvbox-scale-headlines
                             (list :height gruvbox-height-plus-2))))))
    `(markdown-header-face-3
      ((,class (:inherit ,s-variable-pitch :foreground ,red
                         ,@(when gruvbox-scale-headlines
                             (list :height gruvbox-height-plus-1))))))
    `(markdown-header-face-4 ((,class (:inherit ,
                                       s-variable-pitch :foreground ,orange))))
    `(markdown-header-face-5 ((,class (:inherit ,
                                       s-variable-pitch :foreground ,yellow))))
    `(markdown-header-face-6 ((,class (:inherit ,
                                       s-variable-pitch :foreground ,green))))
...
;;;;; org-mode
    `(org-agenda-structure
      ((,class (:foreground ,base1 :background ,base02
                            :weight bold :slant normal :inverse-video nil :height ,gruvbox-height-plus-1
                            :underline nil
    ...
    `(org-level-1 ((,class (:inherit ,s-variable-pitch :foreground ,blue
                                     ,@(when gruvbox-scale-headlines
                                         (list :height gruvbox-height-plus-3))))))
    `(org-level-2 ((,class (:inherit ,s-variable-pitch :foreground ,violet
                                     ,@(when gruvbox-scale-headlines
                                         (list :height gruvbox-height-plus-2))))))
    `(org-level-3 ((,class (:inherit ,s-variable-pitch :foreground ,red
                                     ,@(when gruvbox-scale-headlines
                                         (list :height gruvbox-height-plus-1))))))
    `(org-level-4 ((,class (:inherit ,s-variable-pitch
                                     :foreground ,orange))))
    ...
    `(org-document-title ((,class (:foreground ,base1  :weight bold :height ,gruvbox-height-plus-4))))
...
;;;;; outline
    `(outline-1 ((,class (:inherit org-level-1 :height ,gruvbox-height-plus-4))))
    `(outline-2 ((,class (:inherit org-level-2 :height ,gruvbox-height-plus-3))))
    `(outline-3 ((,class (:inherit org-level-3 :height ,gruvbox-height-plus-2))))
    `(outline-4 ((,class (:inherit org-level-4 :height ,gruvbox-height-plus-1))))
...
;;;;; powerline
    `(powerline-active1 ((,class ,(if gruvbox-high-contrast-mode-line
                                      `(:background ,base00 :foreground ,base03)
                                    `(:background ,base03 :foreground ,base00)))))
    `(powerline-active2 ((,class ,(if gruvbox-high-contrast-mode-line
                                      `(:background ,base01 :foreground ,base03)
                                    `(:background ,base02 :foreground ,base00)))))
    `(powerline-inactive1 ((,class ,(if gruvbox-high-contrast-mode-line
                                        `(:background ,base03 :foreground ,base1)
                                      `(:background ,base02 :foreground ,base01)))))
    `(powerline-inactive2 ((,class ,(if gruvbox-high-contrast-mode-line
...
;;;;; smart-mode-line
    ;; use (setq sml/theme nil) to enable Gruvbox for sml
...
;;;;; sx (WIP)
    ...
    ;; ensures readability as a bg for all gruvbox faces. If it's possible, that is.
...
;;;;;; headings
    `(sr-active-path-face ((,class (:background ,blue :foreground ,base03
                                                :height ,gruvbox-height-plus-1  :weight bold))))
    `(sr-editing-path-face ((,class (:background ,yellow :foreground ,base03
                                                 :weight bold :height ,gruvbox-height-plus-1))))
...
;;;;; weather-metno
    `(weather-metno-date ((,class (:foreground ,yellow :height ,gruvbox-height-plus-3))))
...
;;;;; highlight-symbol
    ...
    `(highlight-symbol-colors
      (--map (gruvbox-color-blend it ,base03 0.25)
...
;;;;; smartrep
    `(smartrep-mode-line-active-bg (gruvbox-color-blend ,green ,s-mode-line-bg 0.2))
;;;;; term
...
;;;;; vc
    `(vc-annotate-color-map
      '((20 . ,red)
        (40 . ,(gruvbox-color-blend yellow red (/ 2.0 4)))
        (60 . ,(gruvbox-color-blend yellow red (/ 3.0 4)))
        (80 . ,yellow)
        (100 . ,(gruvbox-color-blend green yellow (/ 2.0 6)))
        (120 . ,(gruvbox-color-blend green yellow (/ 3.0 6)))
        (140 . ,(gruvbox-color-blend green yellow (/ 4.0 6)))
        (160 . ,(gruvbox-color-blend green yellow (/ 5.0 6)))
        (180 . ,green)
        (200 . ,(gruvbox-color-blend cyan green (/ 2.0 6)))
        (220 . ,(gruvbox-color-blend cyan green (/ 3.0 6)))
        (240 . ,(gruvbox-color-blend cyan green (/ 4.0 6)))
        (260 . ,(gruvbox-color-blend cyan green (/ 5.0 6)))
        (280 . ,cyan)
        (300 . ,(gruvbox-color-blend blue cyan (/ 2.0 5)))
        (320 . ,(gruvbox-color-blend blue cyan (/ 3.0 5)))
        (340 . ,(gruvbox-color-blend blue cyan (/ 4.0 5)))
        (360 . ,blue)))
...
;;; Setup End
  ...
  )    ; END defun create-gruvbox-theme
